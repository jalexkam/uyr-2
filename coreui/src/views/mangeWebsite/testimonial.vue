      ...
      <CRow class="m-0">
         <CCol sm="12" class="px-2 btn-sticky">
            <div class="d-flex justify-content-between py-2 align-items-center">
               <h5 class="mb-0">Testimonials<vue-fontawesome icon="caret-right" class="px-1" size="1"></vue-fontawesome>List</h5>
               <div class="d-flex">
                  <router-link :to="{ name: 'manage-website-add-testimonial' }" >
                     <CButton size="sm" class="btn_custom btn-flex">
                     <vue-fontawesome icon="plus" class="mr-1" size="0.8"></vue-fontawesome>
                     Add New Testimonials</CButton>
                     </router-link>
                     ...
                     <table class="table table-striped table-hover">
                        ...
                        <tbody>
                           <tr class="mb-2 card-shadow" v-for="(row, index) in result.data" :key="'row'+index">
                              ...
                              <td>{{row.name}}</td>
                              <td> <span v-html="row.description"> </span> </td>
                              <td>{{row.created_at}}</td>
                              <td>
                                    <CButtonGroup size="sm">
                                   <router-link :to="{ name: 'manage-website-edit-testimonial',params: { id: row.id } }">
...
<script>
   ...
   export default{
      ...
      computed: {
         ...mapGetters("ManageWebsite/Testimonials", ["result","returnData"]),
      },

      methods: {
         ...mapActions("ManageWebsite/Testimonials", ["list","deleteDoctor","UpdateMultiAction"]),
