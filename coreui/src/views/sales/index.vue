    ...
    <CRow class="m-0">
         <CCol sm="12" class="p-2">
            <div class="d-flex justify-content-between align-items-center">
               <h5 class="mb-0">Sales Manager List</h5>
               <div class="d-flex">
                ...
                <tr v-if="result.data" v-for="(row, index) in result.data" :key="index">
                  ...
                  <td>
                   ...
                   <!--  <router-link :to="{ name: 'changePassword',params: { id: row.id } }">
                       ...
                       <router-link :to="{ name: 'editSales',params: { id: row.id } }">
                       <CButton size="sm" class="btn-outline-success" title="EDIT">
                          <vue-fontawesome icon="pencil" size="0.8"></vue-fontawesome>
                        </CButton>
                      </router-link>

                      <CButton size="sm" color="" class="btn-outline-danger" @click="MultiAction(row.id, 'Delete')" title="SALES DELETE">
...
export default {
  ...
  created: function () {
    ...
    ...mapGetters("Sales/Index", ["result","returnData"]),
  },

 
  methods: {
    ...mapActions("Sales/Index", ["list","UpdateMultiAction"]),

     paginateHandle(pageNum) {
            this.$router.push({ name: 'paginate_sales', params: { page: pageNum } });
        },
        ...
        searchData() {
            ...
            if (this.keyword.length >= 3) {
                if (this.$route.params.page != 1)
                    this.$router.push({ name: 'paginate_sales', params: { page: page } });
                this.list({ page: page, keyword: this.keyword });
            } else {
                this.list({ page: page, keyword: this.keyword });
            }
