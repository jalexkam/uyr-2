         ...
         <CCol sm="12" class="p-2">
            <div class="d-flex justify-content-between align-items-center">
               ...
               <div class="d-flex">
                  ...
                  <span v-if="result.data && result.data.length < 5 ">
                  ...
                  <td>
                    <CButtonGroup size="sm">
                      <!-- <router-link :to="{ name: 'changePassword',params: { id: row.id } }">
                        ...
                        <router-link :to="{ name: 'view_patient' ,params: { id: row.patient_id }}">
                        <CButton size="sm" color="" class="btn-outline-info">
                        <vue-fontawesome icon="eye" size="0.8"></vue-fontawesome>
                        </CButton>
                        </router-link>
                        <router-link :to="{ name: 'edit_patient',params: { id: row.patient_id } }">
                        <CButton size="sm" color="" class="btn-outline-success">
                        <vue-fontawesome icon="pencil" size="0.8"></vue-fontawesome>
                        </CButton>
                        </router-link>
                        <CButton size="sm" color="" class="btn-outline-danger" @click="MultiAction(row.patient_id, 'Delete')">
                        <vue-fontawesome icon="trash" size="0.8"></vue-fontawesome>
                        </CButton>

                      <!-- <router-link :to="{ name: 'Addedit',params: { id: row.id } }">
...
export default {
  ...
  computed: {
    ...
    ...mapGetters("Patient/Index", ["result","returnData"]),
  },
  methods: {
    ...mapActions("Patient/Index", ["list","deleteUser","UpdateMultiAction"]),

     paginateHandle(pageNum) {
            this.$router.push({ name: 'paginate_patient', params: { page: pageNum } });
      ...
      searchData() {
            ...
            if (this.keyword.length >= 3) {
                if (this.$route.params.page != 1)
                    this.$router.push({ name: 'paginate_patient', params: { page: page } });
                this.list({ page: page, keyword: this.keyword });
            } else {
                this.list({ page: page, keyword: this.keyword });
            }
