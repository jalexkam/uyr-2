      ...
      <CRow class="m-0">
         <CCol sm="12" class="px-2 btn-sticky">
            <div class="d-flex justify-content-between py-2 align-items-center">
               <h5 class="mb-0">Home page Slider
                </h5>
               <div class="d-flex">
                     <CButton size="sm" class="btn_custom float-right file_i mr-2">
                        <vue-fontawesome icon="plus" class="mr-1" size="0.8"></vue-fontawesome>Add New Slider
                     ...
                     <table class="table">
                        ...
                        <tbody>
                           <tr class="mb-2 card-shadow" v-for="(row, index) in result.data" :key="'row'+index">
                              ...
                              <td>
                                 <div class="tblogimg">
                                    <img :src="'/uploads/homepageSlider/'+row.id+'/'+row.image" v-if="row.image">
...
<script>
   ...
   export default{
      ...
      computed: {
         ...mapGetters("ManageWebsite/HomepageSlider", ["result","returnData"]),
      },

      methods: {
         ...mapActions("ManageWebsite/HomepageSlider", ["list","UpdateMultiAction","submitForm"]),

          paginateHandle(pageNum) {
            this.$router.push({ name: 'slider_list', params: { page: pageNum } });
