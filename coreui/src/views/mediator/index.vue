      ...
      <CRow class="m-0">
         <CCol sm="12" class="p-2">
            <div class="d-flex justify-content-between align-items-center">
               <h5 class="mb-0">Mediator Doctors List</h5>
              <div class="d-flex">
                  ...
                  <router-link :to="{ name: 'add_mediator' }" >
        ...
        <CRow  class="m-0"  v-if="result.data && result.data.length > 0 && result.total > 0">
         <div class="col-md-4 px-2" v-for="(row, index) in result.data" :key="index">
            <CCard class="doctor_card">
               <div class="doctor_list_card" >
                  ...
                  <div class="doctor_info">
                    ...
                    <router-link :to="{ name: 'view_mediator' ,params: { id: row.id }}">
                        <CButton size="sm" color=""  class="btn-outline-info">
                        <vue-fontawesome icon="eye" size="0.8"></vue-fontawesome>
                        </CButton>
                    </router-link>
                    <router-link :to="{ name: 'edit_mediator' ,params: { id: row.id }}" v-if="user && (user.role_type ==1 || user.role_type == 2)">
...
export default {
  ...
  created: function () {
    ...
    ...mapGetters("Mediator/Index", ["result","returnData"]),
    ...mapGetters("auth", ["user"]),
  },

 
  methods: {
    ...mapActions("Mediator/Index", ["list","deleteDoctor","UpdateMultiAction"]),
     paginateHandle(pageNum) {
            this.$router.push({ name: 'paginate_mediator', params: { page: pageNum } });
        },
        searchData() {
            var page = 1;
            if (this.keyword.length >= 3) {
                if (this.$route.params.page != 1)
                    this.$router.push({ name: 'paginate_mediator', params: { page: page } });
                this.list({ page: page, keyword: this.keyword });
            } else {
                this.list({ page: page, keyword: this.keyword });
            }
