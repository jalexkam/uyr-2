                     ...
                     <table class="table table-striped table-hover">
                        ...
                        <tbody v-if="result.data && result.data.length > 0 && result.total > 0">
                           <tr class="mb-2 card-shadow" v-if="result.data" v-for="(row, index) in result.data" :key="'row'+index">
                            ...
                            <td v-if="row.equipment_name">{{row.equipment_name}}</td>
                            <td v-else></td>
         ...
         <CCol md="3" class="px-2">
          ...
          <CCard>
            <CCardHeader class="p-2 px-3 bg_themes">
              <h6 class="mb-0">{{label}} Equipment</h6>
            </CCardHeader>
            <CCardBody class="p-2">
          <CForm method="POST">
                           <div class="form-group">
                              <label>Equipment Name<span class="text-danger">*</span></label>
                              <CInput placeholder="" v-model="formData.equipment_name"/>
                                 <small class="text-danger" v-if="ajax_error.errors.equipment_name">{{ ajax_error.errors.equipment_name[0] }}</small>
                           </div>
                           <div class="form-group">                        
   ...
   export default {
       components: {
         ...
         data () {
            ...
            keyword    : '',
            label      : 'Add',
            formData     : new Form({ id: "",equipment_name:'',isactive:''}),
            }
         },
     ...
     computed: {
      page() {
       ...
       ...mapGetters("Masters/EquipmentsMaster", ["result","editData","returnData","ajax_error"]),
     },
     methods: {
     
       ...mapActions("Masters/EquipmentsMaster", ["list","edit","submitForm","UpdateMultiAction"]),

        paginateHandle(pageNum) {
          this.list({ page: pageNum,'keyword': this.keyword});
          this.$router.push({ name: 'paginate_equipments', params: { page: pageNum } });
        },
     ...
     .catch(error => {
      ...
      getEquipmentForm(id){
        ...
        resetForm() {
          ...
          this.user_id        = '';
          this.list({ page: this.result.current_page});
          this.formData =  new Form({ id: "",equipment_name:'',isactive: ""});
        },
         searchData() {
            var page = 1;
            if (this.keyword.length >= 3) {
                if (this.$route.params.page != 1)
                    this.$router.push({ name: 'equipments', params: { page: page } });
                this.list({ page: page, keyword: this.keyword });
            } else {
                this.list({ page: page, keyword: this.keyword });
            }
