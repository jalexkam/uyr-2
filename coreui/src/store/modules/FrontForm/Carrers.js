...
export const actions = {
  ...
  list: ({ commit, dispatch }, payload) => {
    ...
    axios.get("/api/careers?page=" + payload.page + '&keyword=' + payload.keyword)
      .then(response => {
  ...
  edit: ({ commit }, payload) => {
    ...
    UpdateMultiAction: ({ commit, dispatch }, payload) => {
      return new Promise((resolve, reject) => {
          axios.delete('/api/careers/' + payload.id + '?action=' + payload.action)
                .then((response) => {
    ...
    UpdateStatus: ({ commit, dispatch }, payload) => {
      return new Promise((resolve, reject) => {
          axios.get('/api/careers/updateStatus/' + payload.id + '?status=' + payload.status)
                .then((response) => {
                  commit(types.MULTI_ACTION, response.data);
                  resolve();
              })
