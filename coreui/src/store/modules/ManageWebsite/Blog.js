...
export const mutations = {
  [types.MASTER_BLOG_LIST](state, payload) {
    state.resultData  = payload;
  },
  [types.MASTER_BLOG_DATA](state, payload) {
    state.editArr  = payload;
  },
  [types.MASTER_CREATE_BLOG](state, payload) {
...
export const actions = {
  // Contact Us LIST
   list: ({ commit, dispatch }, payload) => {
    ...
    axios.get("/api/manageWebsite/blog?page=" + payload.page + '&keyword=' + payload.keyword)
      .then(response => {
        commit(types.MASTER_BLOG_LIST, response.data);
      })
      .catch(error => {
  ...
  edit: ({ commit }, payload) => {
        return new Promise((resolve, reject) => {
            axios
                .get('/api/manageWebsite/blog/' + payload)
                .then((response) => {
                    commit(types.MASTER_BLOG_DATA, response.data);
                    resolve();
                })
                .catch((error) => {
 ...
 UpdateMultiAction: ({ commit, dispatch }, payload) => {
      return new Promise((resolve, reject) => {
          axios
              .delete('/api/manageWebsite/blog/' + payload.id + '?action=' + payload.action)
              .then((response) => {
 ...
 submitForm: ({ commit, dispatch }, payload) => {
    return new Promise((resolve, reject) => {
        if (payload.id == '') {
            var post_url = axios.post('/api/manageWebsite/blog', payload.newData);
        } else {
            var post_url = axios.post('/api/manageWebsite/blog/update/' + payload.id, payload.newData);
        }
        return post_url
            .then((response) => {
                if (response.data.status == 'error') {
                    ...
                    commit(types.MASTER_CREATE_BLOG, response.data);
                    resolve();
                }
            })
            .catch((error) => {
