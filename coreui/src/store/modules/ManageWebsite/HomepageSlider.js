...
export const actions = {
  // Contact Us LIST
   list: ({ commit, dispatch }, payload) => {
    ...
    axios.get("/api/manageWebsite/homepage_slider?page=" + payload.page + '&keyword=' + payload.keyword)
      .then(response => {
  ...
  edit: ({ commit }, payload) => {
        return new Promise((resolve, reject) => {
            axios
                .get('/api/manageWebsite/homepage_slider/' + payload)
                .then((response) => {
 ...
 UpdateMultiAction: ({ commit, dispatch }, payload) => {
      return new Promise((resolve, reject) => {
          axios
              .delete('/api/manageWebsite/homepage_slider/' + payload.id + '?action=' + payload.action)
              .then((response) => {
 ...
 submitForm: ({ commit, dispatch }, payload) => {
    return new Promise((resolve, reject) => {
        if (payload.id == '') {
            var post_url = axios.post('/api/manageWebsite/homepage_slider', payload.newData);
        } else {
            var post_url = axios.post('/api/manageWebsite/homepage_slider/update/' + payload.id, payload.newData);
        }
        return post_url
            .then((response) => {
