...
export const actions = {
  ...
  list: ({ commit, dispatch }, payload) => {
    ...
    axios.get("/api/users?page=" + payload.page + '&keyword=' + payload.keyword)
      .then(response => {
  ...
  UpdateMultiAction: ({ commit, dispatch }, payload) => {
      return new Promise((resolve, reject) => {
          axios
              .get('/api/users/delete/' + payload.id + '?action=' + payload.action)
              .then((response) => {
  ...
  //ADD AND EDIT FORM
      submitForm: ({ commit, dispatch }, payload) => {
        return new Promise((resolve, reject) => {
            if (payload.id == '') {
                var post_url = axios.post('/api/users', payload);
            } else {
                var post_url = axios.patch('/api/users/' + payload.id, payload);
            }
            return post_url
                .then((response) => {
